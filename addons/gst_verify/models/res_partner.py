...
class Partner(models.Model):
    ...
    def validate_gstn_from_master_india(gstin_num):
        ...
        headers = {
            'client_id': '',
            'Content-type': 'application/json',
            'Authorization': 'Bearer %s' % (Partner.get_master_india_access_token())
        }
    ...
    def do_stuff(self):
        try:
            ...
            self.zip = str(gst_data["data"]["pradr"]["addr"]["addr"]["pncd"]) if gst_data["data"]["pradr"]["addr"]["addr"]["pncd"] is not None else None
